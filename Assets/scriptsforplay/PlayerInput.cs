...
public class PlayerInput : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Quaternion rot = Quaternion.FromToRotation(Vector3.up, collision.contact.normal);
        Vector3 pos = collision.contact.point;
        GameObject judgeNow=Instantiate(judgePrefab[timeVal], pos, rot);
       // Destroy(judgeNow);//이거 언제 해야 에니메이션 끝나고 없어지는지 돌렵보고 판단 해야할 듯? -> 근데 최대 15개 가량이니까 냅두고 싶기도 하지만 그러지 말자!
